...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (error) {
      console.error("Supabase icp GET error:", error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ icps: data });
  } catch (e) {
    console.error("ICP GET exception:", e);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (error) {
      console.error("Supabase icp POST error:", error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ icp: data });
  } catch (e) {
    console.error("ICP POST exception:", e);
    return NextResponse.json({ error: errMessage(e) }, { status: 500 });
  }
}
